...
internal fun annotationsByClassId(
    ...
    if (firSymbol.isFromCompilerRequiredAnnotationsPhase(classId, builder.rootSession)) {
        ...
        return annotationContainer.resolvedCompilerRequiredAnnotations(firSymbol).mapIndexedNotNull { index, annotation ->
            ...
            annotation.toKaAnnotation(builder, index) { annotationCall ->
                computeAnnotationArguments(annotationCall, builder)
            }
        }
    }
...
internal fun computeAnnotationArguments(annotationCall: FirAnnotationCall, builder: KaSymbolByFirBuilder): List<KaNamedAnnotationValue> {
    if (annotationCall.arguments.isEmpty()) return emptyList()

    val symbol = annotationCall.containingDeclarationSymbol
    if (symbol.fir.resolvePhase < FirResolvePhase.ANNOTATION_ARGUMENTS) {
        when (annotationCall.toAnnotationClassId(builder.rootSession)) {
            StandardClassIds.Annotations.Target -> return computeKotlinTargetAnnotationArguments(annotationCall, builder)
            JvmStandardClassIds.Annotations.Java.Target -> return computeJavaTargetAnnotationArguments(annotationCall, builder)
        }
    }
    ...
    return FirAnnotationValueConverter.toNamedConstantValue(
        ...
        mapAnnotationParameters(annotationCall),
        builder
    )
}
