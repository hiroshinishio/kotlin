...
abstract class AndroidSdkProvisionerExtension {
    private val providedSdkTypes: MutableMap<TaskName, ProvisioningType> = mutableMapOf()
    ...
    fun Task.acceptLicenses(acceptLicensesTaskName: TaskName = "acceptAndroidSdkLicenses") {
        dependsOn(maybeRegisterAcceptLicensesTask(acceptLicensesTaskName))
    }
    ...
    private fun Task.getSdkPath(provisioningType: ProvisioningType): Provider<String> {
        ...
        providedSdkTypes[name] = provisioningType
    ...
    private fun Task.maybeRegisterAcceptLicensesTask(taskName: String): TaskProvider<*> {
        val task =
            if (taskName in project.tasks.names)
                project.tasks.named(taskName)
            else
                project.tasks.register(taskName) {
                    val androidSdkConfiguration = project.configurations.getByName(ProvisioningType.SDK.configurationName)
        ...
        task.configure {
            val taskRequestedSdkType = project.provider {
                providedSdkTypes[name]
            }
            doFirst {
                require(taskRequestedSdkType.get() == ProvisioningType.SDK) {
                    "Task $path didn't request Android SDK with provisioning type ${ProvisioningType.SDK}, but it's requesting accepting the licence. This is not supported."
                }
            }
        }
        return task
